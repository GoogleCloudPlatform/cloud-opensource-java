...
public class LinkageChecker {
  ...
  private final ImmutableList<AnnotatedJar> jars;
  ...
  public static LinkageChecker create(List<AnnotatedJar> jars, Iterable<AnnotatedJar> entryPoints)
      throws IOException {
  ...
  public static LinkageChecker create(Bom bom) throws IOException {
    ...
    ImmutableList<AnnotatedJar> classpath = classPathResult.getClassPath();

    // When checking a BOM, entry point classes are the ones in the artifacts listed in the BOM
    List<AnnotatedJar> artifactJarsInBom = classpath.subList(0, managedDependencies.size());
    ImmutableSet<AnnotatedJar> entryPoints = ImmutableSet.copyOf(artifactJarsInBom);
        ...
        (classFile, classSymbol) -> {
          if (classSymbol instanceof SuperClassSymbol) {
            ...
            if (!problems.isEmpty()) {
              ...
              AnnotatedJar superClassLocation = classDumper.findClassLocation(superClassName);
          ...
          if (!classDumper
            ...
            if (classSymbol instanceof InterfaceSymbol) {
              ...
              if (!problems.isEmpty()) {
                ...
                AnnotatedJar interfaceLocation = classDumper.findClassLocation(interfaceName);
  ...
  Optional<SymbolProblem> findSymbolProblem(ClassFile classFile, MethodSymbol symbol) {
    ...
    try {
      ...
      AnnotatedJar classFileLocation = classDumper.findClassLocation(targetClassName);
