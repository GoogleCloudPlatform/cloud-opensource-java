 ...
 *
 * <p>A cycle in a dependency graph is a situation where a path to a node from the root contains
 * the same node.
 */
final class CycleBreakerGraphTransformer implements DependencyGraphTransformer {
  ...
  private void removeChildFromParent(DependencyNode parent, DependencyNode node) {
    ImmutableList<DependencyNode> children = parent.getChildren().stream().filter(child -> child != node).collect(
