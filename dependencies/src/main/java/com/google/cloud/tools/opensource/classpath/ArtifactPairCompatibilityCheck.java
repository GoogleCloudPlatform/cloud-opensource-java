...
class ArtifactPairCompatibilityCheck {
    public static void main(String[] arguments) throws IOException, ArtifactDescriptorException {
        ...
        Bom bom = RepositoryUtility.readBom(arguments[1]);
        ...
        ImmutableSetMultimap<SymbolProblem, ClassFile> artifactInherentProblems = filterReachable(artifactChecker.findSymbolProblems(), artifactChecker);
        ...
        for (Artifact bomMember: bom.getManagedDependencies()) {
            if (!bomMember.getArtifactId().startsWith("google-cloud")) {
                // Let's focus google-cloud-XXXX libraries
                continue;
            }
            Set<Entry<SymbolProblem, ClassFile>> intrinsicErrors = new HashSet<>(
                    artifactInherentProblems.entries()
            );
            LinkageChecker bomMemberChecker = linkageCheckerOf(ImmutableList.of(bomMember));

            ImmutableSetMultimap<SymbolProblem, ClassFile> bomMemberInherentProblems = filterReachable(bomMemberChecker.findSymbolProblems(), bomMemberChecker);
            ...
            ImmutableSetMultimap<SymbolProblem, ClassFile> canaryProjectProblems = filterReachable(canaryProjectChecker.findSymbolProblems(), canaryProjectChecker);
            ...
            System.out.println(String.format(
                    ...
                    artifact.toString(), bomMember.toString(),
                    canaryOnlyErrors.keySet().size()
            ));
            ...
            System.out.println(String.format("### Conflicting: %d, Non-conflicting: %d", conflictingArtifactCount, nonConflictingArtifactCount));
