...
public class RepositoryUtilityTest {
  ...
  public void testReadBom_path() throws MavenRepositoryException, IOException {
    ...
    Iterable<String> expectedArtifacts =
        filter(
            readLines(getResource(RepositoryUtilityTest.class, GOLDEN), UTF_8),
          ...
          .writeLines(
              Stream.concat(
                  Stream.of(
                      ...
                      ""),
                  currentBom.getManagedDependencies().stream()
                      .map(Artifacts::toCoordinates)
                      .sorted()));
      fail(
          String.format(
              "Wrote new golden file at %s.\nNow rerun the test without setting %s.",
              goldenFile, UPDATE_GOLDEN_ARTIFACTS));
    }
  }
